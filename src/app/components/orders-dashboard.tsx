...
function CreateOrderPanel({ onCreated }: { onCreated: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) { const body = await response.json().catch(() => null) as { error?: { message?: string } } | null; setError(body?.error?.message ?? "Please check the order details."); return; }
...
export default function OrdersDashboard() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      const result = (await response.json()) as { items: Order[]; pagination: typeof pagination };
  ...
  const cards = useMemo(() => ({
    outstanding: orders.reduce((sum, order) => sum + Number(order.amountDue), 0),
