...
The deployment role trusts only this repository’s `main` branch through GitHub OIDC. Scope it to ECR push actions for `crossval-orders` and `ecs:DescribeExpressGatewayService`/`ecs:UpdateExpressGatewayService` for the Express service. The workflow polls the service’s current image and public health endpoints directly, rather than granting broad deployment-monitoring permissions. It does not receive Atlas credentials, schema-admin privileges, or a long-lived AWS access key. The ECS task keeps `MONGODB_URI` in Secrets Manager; GitHub never receives the secret value.
